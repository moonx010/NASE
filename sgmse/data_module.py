...
class Specs_noise_label(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
            format='default', normalize="noisy", spec_transform=None,
            stft_kwargs=None, **ignored_kwargs):
        ...
        if format == "default":
            ...
            noise_labels = []
            with open(join(data_dir, subset) + '/noise_label.txt') as f:
                for line in f.readlines():
                    noise_labels.append(line.strip().split()[:2])
            self.noise_labels = sorted(noise_labels, key = lambda x : x[0])
    ...
    def __getitem__(self, i):
        assert self.noise_labels[i][0] in self.noisy_files[i], (self.noise_labels[i], self.noisy_files[i])
        x, _ = load(self.clean_files[i])
        y, _ = load(self.noisy_files[i])
        noise_label = torch.tensor(noise2id[self.noise_labels[i][1]])
